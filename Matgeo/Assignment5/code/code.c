...
void generate_plane(float a[3], float b[3], float u_min, float u_max, float v_min, float v_max, int steps, int plane_id) {
    ...
    // Print the generated points (x, y, z) with plane_id to distinguish
    for (int k = 0; k < total_points; k++) {
        printf("%d %.2f %.2f %.2f\n", plane_id, points[k][0], points[k][1], points[k][2]);
    }
...
int main() {
    ...
    generate_plane(a, b, u_min, u_max, v_min, v_max, steps, 1);

    // Generate points for the second plane using vectors b and c
     generate_plane(b, c, u_min, u_max, v_min, v_max, steps, 2);
